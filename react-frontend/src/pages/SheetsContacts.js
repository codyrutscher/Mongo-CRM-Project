  ...
  const loadContacts = async () => {
    try {
      ...
      if (Object.keys(filters).length > 0 || searchQuery) {
        ...
        const searchFilters = { ...filters, source: 'google_sheets' };
        if (searchQuery) {
  ...
  const createSegmentFromSelected = async () => {
    ...
    try {
      ...
      const response = await createSegment({
        name: segmentName,
        description: `Google Sheets segment with ${contactIds.length} selected contacts`,
        ...
        <Row>
          <Col>
            <div className="text-center py-4">
              ...
              <p className="mt-2">Loading Google Sheets contacts...</p>
            </div>
          </Col>
        </Row>
      )}
        ...
        <Row className="mb-3">
          <Col>
            <div className="text-muted">
              {totalRecords > 0 ? (
                <>
                  Showing {((currentPage - 1) * pageSize) + 1} - {Math.min(currentPage * pageSize, totalRecords)} of {totalRecords} Google Sheets contacts
                  {Object.keys(filters).length > 0 && <span className="text-primary"> (filtered)</span>}
                </>
              ) : (
                'No Google Sheets contacts found'
              )}
            </div>
          </Col>
        </Row>
        ...
        <Col>
          {!loading && contacts.length === 0 ? (
            <div className="text-center py-5">
              ...
              <h5 className="text-muted">No Google Sheets contacts found</h5>
              <p className="text-muted">
                {Object.keys(filters).length > 0 || searchQuery 
                  ? 'Try adjusting your search criteria or filters.'
                  : 'No contacts have been imported from Google Sheets yet.'
                }
              </p>
            </div>
          ) : viewType === 'grid' ? (

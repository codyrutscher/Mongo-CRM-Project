...
import { getContactsWithFilters, getContact, getAllFilteredContactIds, createSegment } from '../services/api';

const CSVSourceContacts = () => {
  ...
  const displayName = decodedSource.replace('csv_', '').replace(/_/g, ' ');

  useEffect(() => {
    loadContacts();
  }, [currentPage, pageSize, sortField, sortOrder, source]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const loadContacts = async () => {
    try {
      ...
      const response = await getContactsWithFilters(
        searchFilters, 
        currentPage, 
        pageSize, 
        sortField, 
  ...
  const selectAllFilteredResults = async () => {
    try {
      setLoading(true);
      const searchFilters = { source: decodedSource };
      if (searchQuery) {
        searchFilters.searchQuery = searchQuery;
      }
      ...
      if (response.data.success) {
        ...
        alert(`Selected ALL ${allContactIds.length} contacts from this CSV!`);
      }
    } catch (error) {
      console.error('Error selecting all contacts:', error);
      alert('Failed to select all contacts');
    } finally {
      setLoading(false);
    }
  };
  ...
  const createSegmentFromSelected = async () => {
    ...
    const segmentName = prompt(
      `Create segment from ${selectedContacts.size} selected contacts from "${displayName}".\n\nEnter segment name:`
    );
    ...
    try {
      ...
      const response = await createSegment({
        name: segmentName,
        description: `Segment from ${displayName} CSV with ${contactIds.length} selected contacts`,
        ...
        <Col>
          <div className="d-flex align-items-center mb-4">
            <Button
              ...
              className="me-3"
            >
              <i className="fas fa-arrow-left"></i> Back to CSV Index
            </Button>
            <div>
              <h2 className="mb-1">
                <i className="fas fa-file-csv"></i> {displayName}
              </h2>
              <p className="text-muted mb-0">
                Contacts from this CSV upload
              </p>
            </div>
          </div>
        </Col>
          ...
          <Col>
            <div className="d-flex gap-2">
              <Button
                ...
                disabled={loading}
              >
                <i className="fas fa-check-square"></i> Select All {totalRecords} Results
              </Button>
            ...
            <div className="text-muted">
              {totalRecords > 0 ? (
                <>
                  ...
                  {totalRecords} contacts from {displayName}
                  {searchQuery && (
                    <span className="text-primary"> (filtered)</span>
                  )}
                </>
              ) : (
                `No contacts found in ${displayName}`
              )}
            </div>
          </Col>
        </Row>
            ...
            <div className="text-center py-5">
              ...
              <p className="text-muted">
                {searchQuery
                  ...
                  : `No contacts have been uploaded to ${displayName} yet.`}
              </p>
            </div>
          ) : viewType === "grid" ? (
            <Row>
...
export default CSVSourceContacts;
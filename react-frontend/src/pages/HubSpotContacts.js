  ...
  const loadContacts = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        console.log('Contacts loaded:', response.data.data?.length, 'Total pages:', response.data.pagination?.totalPages, 'Total records:', response.data.pagination?.totalRecords);
      }
    } catch (error) {
  ...
  const createSegmentFromSelected = async () => {
    ...
    try {
      ...
      const response = await createSegment({
        name: segmentName,
        description: `HubSpot segment with ${contactIds.length} selected contacts`,
